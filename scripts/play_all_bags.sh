...
if [[ -f "$PREPARED_BAG/metadata.yaml" ]]; then
  if ((LIO_ONLY)); then
    prepared_topic_args=(--topics front_lidar imu_data imu_data/odometry)
  else
    prepared_topic_args=(--topics front_lidar imu_data imu_data/odometry midrange_camera/ffmpeg)
  fi
...
run_stream() {
  ...
  if ((PLAY_ALL_TOPICS)); then
    ...
    topic_args=(--topics front_lidar imu_data imu_data/odometry)
  fi
